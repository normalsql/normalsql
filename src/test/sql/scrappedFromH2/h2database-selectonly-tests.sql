...
skip SELECT 1 IN (SELECT * FROM TABLE(X NULL=()));
SELECT (1, 1) IN (VALUES (1, NULL));
SELECT (1, 1) IN (VALUES (NULL, 1));
skip SELECT (1, 1) IN (SELECT * FROM TABLE(X INT=(), Y INT=()));
...
SELECT * FROM TEST AS T(A, B);
skip SELECT * FROM TEST AS T(A, B) USE INDEX (TEST_I_IDX);
...
skip SELECT str FROM FINAL TABLE (UPDATE test SET str = char(rownum + 48) WHERE str = '0');
...
skip select * from table(a int=(1)), table(b int=2), table(c int=row(3));
SELECT * FROM (SELECT * FROM TEST) ORDER BY id;
SELECT * FROM (SELECT * FROM TEST) x ORDER BY id;
skip select * from table(id int = (1));
skip select * from table(id int=(1, 2), name varchar=('Hello', 'World')) x order by id;
skip SELECT * FROM (TABLE(ID INT = (1, 2)));
...
skip select insert(null, null, null, null) en, insert('Rund', 1, 0, 'o') e_round, insert(null, 1, 1, 'a') ea;
skip select insert('World', 2, 4, 'e') welt, insert('Hello', 2, 1, 'a') hallo;
skip SELECT INSERT(NULL, 0, 0, NULL);
